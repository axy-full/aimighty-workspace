...
export function SuiteAgentPanel({ suite, project, scope, enabled = true, onSave, onApply, onQueued }: {
  ...
  return <section className={styles.panel} aria-label={copy.title}>
    ...
    <footer className={styles.footer}><span>Claude or ChatGPT · model and effort selected with the quote</span><button className={styles.primary} disabled={!active || !configured || live || draft.value.request.trim().length < 3} onClick={() => setOpen(true)}>{live ? 'Agent working…' : 'Review agent quote'} <ArrowUpRight size={15} /></button></footer>
    {state.error && <p className={styles.error} role="alert">{state.error}</p>}
    {active && state.data && !configured && <p className={styles.error}>Connect a priced Claude or OpenAI model in Workspace → Engines.</p>}
