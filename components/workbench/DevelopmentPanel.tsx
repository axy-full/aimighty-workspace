...
export function DevelopmentPanel({ project, kind, scope, enabled, models: connectedModels, onSave, onApply }: {
  ...
  return <section className={styles.panel} aria-label={kindLabel(kind)}>
    ...
    <div className={styles.providers} role="group" aria-label={`${kindLabel(kind)} provider`}>
      <button type="button" aria-pressed={provider === 'anthropic'} disabled={!!busy || !!pending} onClick={() => { setProvider('anthropic'); setPickedModel(''); setEffort('auto'); }}>Claude <small>Anthropic</small></button>
      <button type="button" aria-pressed={provider === 'openai'} disabled={!!busy || !!pending} onClick={() => { setProvider('openai'); setPickedModel(''); setEffort('auto'); }}>ChatGPT <small>OpenAI models</small></button>
    </div>
    ...
    {!enabled && <p className={styles.hint}>Sign in and save a project to use Claude or ChatGPT.</p>}
