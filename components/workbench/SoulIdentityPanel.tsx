...
export function SoulIdentityPanel({ project, subjectType, assetId, scope, enabled, onSave, onUpload, onAttach, onClose, onSettings }: {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (alive.current && started === epoch.current) setError(cause instanceof Error ? cause.message : 'Identities could not be loaded.');
    } finally {
  ...
  async function upload(files: File[]) {
    ...
    try {
      if (files.some(file => !file.type.startsWith('image/'))) throw new Error('Choose image files for identity portraits.');
  ...
  async function attach(identity: SoulIdentity) {
    ...
    catch (cause) { if (alive.current) setError(cause instanceof Error ? cause.message : 'This identity could not be attached.'); }
  ...
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}>
    <DialogContent className={`ps ps-dialog ${styles.panel}`} overlayClassName="z-[100]" style={{ zIndex: 101 }}>
      ...
      <div className={styles.body}>
        {!enabled && <div className={styles.notice}>Sign in and open a saved project to create or use an identity.</div>}
        ...
        <div className={styles.identities} aria-label="Workspace identities">
          {state?.identities.length === 0 && <p className={styles.help}>No identities yet. Train a character from your project portraits below.</p>}
          {state?.identities.map(identity => <article key={identity.id} className={styles.identity} aria-label={`Identity: ${identity.name}`}>
        ...
        <section className={styles.create} aria-label="Create identity">
          ...
          {(creating || !state?.identities.length) && !paid.pending && <form onSubmit={event => { event.preventDefault(); void submit(); }}>
            <fieldset className={styles.formFields} disabled={disabled || !state?.configured}>
              ...
              <div className={styles.references} role="group" aria-label="Identity portrait references">
