...
function SourceEditor({ project, enabled, onChange, onSave, error, busy, setError, setBusy }: { project: Project; enabled: boolean; onChange: (patch: Partial<Project>) => void; onSave: () => Promise<boolean>; error: string; busy: boolean; setError: (value: string) => void; setBusy: (value: boolean) => void }) {
  ...
  const [name, setName] = useState(source?.name ?? 'Native Blender scene');
  ...
  return <section className={styles.panel} aria-label="Native Blender source"><h3>Native Blender source</h3><p>Keep procedural geometry, rigs, material nodes and animation editable in Blender. The native worker runs the saved source; the visual viewport remains a separate scene preview.</p>
