...
export function AstraExportPanel({ project, scope, enabled, onSave }: { project: Project; scope: string; enabled: boolean; onSave: () => Promise<boolean> }) {
  ...
  async function exportBlender() {
    ...
    try {
      ...
      downloadFile(new Blob([bytes], { type: 'application/zip' }), 'astra-blender.zip');
    } catch (e) { setError((e as Error).message); }
    finally { setBusy(false); }
  }
  return <section className={styles.panel}><h3>Continue in Blender</h3><p>Download the scene, original assets and a build script for Blender 5. Run it locally to create an editable .blend, a Cycles still and a GLB.</p><button className={`${styles.button} ${styles.primary}`} disabled={!enabled || busy} onClick={() => void exportBlender()}>{busy ? 'Collecting originals…' : 'Download Blender package'}</button><p className={styles.notice}>This package runs in your own Blender installation. Native cloud rendering is available separately below when its runtime is connected. The viewport and PNG export run in your browser.</p>{error && <p role="alert" className={styles.error}>{error}</p>}</section>;
}
