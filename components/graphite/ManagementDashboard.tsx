...
/* `spend` is the vendor's dollars, sent only to a workspace that pays its
   vendors in them; a credit workspace reads credits alone. */
type Analytics = {
...
export function ManagementDashboard() {
  ...
  /* One cost column, in the workspace's own unit. */
  const cost = useCallback((r: { spend?: number; credits: number }) => (credits ? `${int(r.credits)} cr` : usd(r.spend ?? 0)), [credits]);
  const t = data?.totals;
  const it = data?.patterns;
  const scopeName = projectId === "all" ? "All projects" : projects.find((p) => p.id === projectId)?.name ?? "This project";
  const exportCsv = useMemo(() => () => {
    if (!data) return;
    const amount: [string, string] = credits ? ["credits", "Credits"] : ["spend", "Cost (USD)"];
    const parts = [
      `# ${scopeName} · ${PERIODS.find((p) => p.days === days)?.label}`,
      "# By project", csv(data.byProject as unknown as Row[], [["name", "Project"], ["n", "Generations"], amount, ["failed", "Failed"], ["people", "People"], ["renderMs", "Render ms"]]),
      "", "# By person", csv(data.byPerson as unknown as Row[], [["name", "Person"], ["email", "Email (masked)"], ["n", "Generations"], amount, ["failed", "Failed"], ["projects", "Projects"]]),
      "", "# By model", csv(data.byModel as unknown as Row[], [["label", "Model"], ["n", "Generations"], amount, ["failed", "Failed"], ["avgMs", "Average ms"]]),
      "", "# Revisions per shot", csv(data.byShot as unknown as Row[], [["code", "Shot"], ["title", "Title"], ["takes", "Takes"], ["ok", "Succeeded"], ["failed", "Failed"], amount]),
    ].join("\n");
      ...
      {t ? (
        <>
          <div className="mdx-tiles" data-testid="dash-totals">
            {credits
              ? <div className="mdx-tile"><span className="gx-eyebrow">Cost</span><strong data-testid="dash-cost">{int(t.credits)} cr</strong><span className="cw-dim">billed to the workspace</span></div>
              : <div className="mdx-tile"><span className="gx-eyebrow">Cost</span><strong data-testid="dash-cost">{usd(t.spend ?? 0)}</strong><span className="cw-dim">vendor cost{t.promptSpend ? ` · ${usd(t.promptSpend)} prompt writing` : ""}</span></div>}
          ...
          {projectId === "all" ? (
            <Table caption="By project · click one to focus the dashboard on it" testid="dash-projects" rows={data!.byProject} onPick={(r) => r.id && setProjectId(r.id)}
              columns={[{ label: "Project", value: (r) => r.name }, { label: "Generations", value: (r) => int(r.n), numeric: true }, { label: "Cost", value: cost, numeric: true },
          ...
          <Table caption="By person" testid="dash-people" rows={data!.byPerson}
            columns={[{ label: "Person", value: (r) => (r.email ? `${r.name} · ${r.email}` : r.name) }, { label: "Generations", value: (r) => int(r.n), numeric: true }, { label: "Cost", value: cost, numeric: true },
          ...
          <Table caption="By model" testid="dash-models" rows={data!.byModel}
            columns={[{ label: "Model", value: (r) => r.label }, { label: "Generations", value: (r) => int(r.n), numeric: true }, { label: "Cost", value: cost, numeric: true },
          ...
          <Table caption="Revisions per shot · the shots that took the most takes" testid="dash-shots" rows={data!.byShot.slice(0, 25)}
            columns={[{ label: "Shot", value: (r) => [r.code, r.title].filter(Boolean).join(" · ") || "—" }, { label: "Takes", value: (r) => int(r.takes), numeric: true },
              { label: "Made", value: (r) => int(r.ok), numeric: true }, { label: "Failed", value: (r) => int(r.failed), numeric: true }, { label: "Cost", value: cost, numeric: true }]} />
          {data!.stuck ? (
          ...
          {data!.byCategory?.length ? (
            <Table caption="By category" testid="dash-categories" rows={data!.byCategory}
              columns={[{ label: "Category", value: (r) => r.category }, { label: "Projects", value: (r) => int(r.projects), numeric: true }, { label: "Generations", value: (r) => int(r.n), numeric: true },
                { label: "Cost", value: cost, numeric: true }, { label: "Failed", value: (r) => pct(r.failed, r.n), numeric: true }, { label: "Average time", value: (r) => secs(r.avgMs), numeric: true }]} />
          ) : null}
          {it ? (
