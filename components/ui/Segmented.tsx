...
export default function Segmented<T extends string>({ value, options, onChange, label, placement = "page", fill = false, className = "" }: Props<T>) {
  return (
    <div role="group" aria-label={label} className={`${fill ? "flex w-full" : "inline-flex"} gap-[2px] rounded-pill border border-border bg-card p-[2px] ${className}`}>
      {options.map((o) => {
        const on = o.value === value;
        return (
          <button key={o.value} type="button" aria-pressed={on} onClick={() => onChange(o.value)}
            className={`tap44 rounded-pill text-[13px] font-medium leading-none ${fill ? "flex min-h-[40px] flex-1 items-center justify-center px-[8px]" : PAD[placement]} ${on ? "bg-selected text-ink" : "text-ink-body"}`}>
            {o.label}
          </button>
        );
      })}
