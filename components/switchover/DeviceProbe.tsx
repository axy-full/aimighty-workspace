 ...
 *
 * Mounted by the ROOT LAYOUT, and nowhere else. React never executes a <script>
 * it creates during a client render — it logs an error saying so, which is both
 * noise and a dev-overlay issue (`tests/customer.spec.ts` counts those). The
 * gate itself is a client component that re-renders (pending → decided), and a
 * page segment is re-rendered by a client-side navigation into it; both were
 * tried and both produced that error under Turbopack, which is what CI runs.
 * The root layout is the one place React renders exactly once per document.
 *
