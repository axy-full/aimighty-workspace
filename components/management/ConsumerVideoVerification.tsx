...
export default function ConsumerVideoVerification() {
  ...
  return <section className="rounded-xl border border-line p-4 space-y-3" aria-label="Marketing Video verification">
    ...
    {job && <>
      ...
      {job.status === "quoted" && <>
        <p className="text-xs text-mute">{job.quoteExpiresAt > clock ? "This quote expires in five minutes. The connected account bills its active workspace, which is shared across its connected clients. Particl checks the wallet and price again before sending." : "This quote expired. Get a fresh quote before generating."}</p>
        <label className="flex items-start gap-2 text-xs leading-relaxed"><input type="checkbox" checked={walletReviewed} disabled={!!busy} onChange={event => setWalletReviewed(event.target.checked)} />Charge {job.quoteCredits} connected credits to {job.workspaceName} for this one test.</label>
      ...
      {(providerStatus || job.providerReceipt || job.result) && <details><summary className="cursor-pointer text-xs text-mute">Verification result details</summary><pre aria-label="Connected account verification result" className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-chip p-3 text-[11px]">{JSON.stringify(providerStatus ?? job.result ?? job.providerReceipt, null, 2)}</pre></details>}
    </>}
