...
export default function HiggsfieldConsumerConnection() {
  ...
  async function act(kind: "connect" | "discover" | "disconnect") {
    if (pending.current) return;
    pending.current = true; setBusy(kind); setProblem(""); setNotice(""); setDismissedOutcome(true);
    try {
      const response = await scopedFetch(`/api/higgsfield/consumer/${kind === "connect" ? "connect" : kind === "discover" ? "capabilities" : "connection"}`, { method: kind === "disconnect" ? "DELETE" : "POST" });
      ...
      if (kind === "connect") {
        ...
        setDiscovery(null); setNotice("Higgsfield consumer connection removed from this Particl workspace."); await refresh();
      }
