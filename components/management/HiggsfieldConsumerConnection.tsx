...
export default function HiggsfieldConsumerConnection() {
  ...
  async function act(kind: "connect" | "discover" | "qualify" | "analysis" | "disconnect") {
    ...
    try {
      ...
      if (kind === "connect") {
        ...
        if (result.readOnly !== true || result.scope !== "analysis-models" || !Array.isArray(result.results)) throw new Error("Higgsfield did not return usable analysis model definitions.");
        ...
        setDiscovery(null); setQualification(null); setAnalysisQualification(null); setNotice("Higgsfield consumer connection removed from this Particl workspace."); await refresh();
      }
  ...
  return <ManagementCard title="Higgsfield · Marketing account" description="Connect your Higgsfield account to discover its consumer Marketing Video, brand, ad-reference and Virality Predictor tools. This is separate from the Cloud API key above.">
    <div className="p-5 space-y-4">
      ...
      {analysisQualification && <details><summary className="cursor-pointer text-xs text-mute">Analysis model diagnostics</summary><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-chip p-3 text-[11px]" aria-label="Higgsfield analysis model definitions">{JSON.stringify(analysisQualification, null, 2)}</pre></details>}
