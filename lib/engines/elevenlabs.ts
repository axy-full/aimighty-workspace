...
import { elevenConfigured, textToSpeech, soundEffect, composeMusic, speechCredits, sfxCredits, musicCredits, usdForCredits } from "../elevenlabs";
...
export const elevenlabs: EngineAdapter = {
  ...
  estimate(req) {
    ...
    const credits = req.task === "speech" ? speechCredits(req.text, req.modelId) : req.task === "sound" ? sfxCredits() : musicCredits(Number(req.params.lengthMs ?? 0));
