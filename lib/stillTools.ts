...
export const STILL_TOOLS: { id: StillTool; label: string; blurb: string; modelId: string }[] = [
  { id: "upscale", label: "Upscale", blurb: "Enhance a source image with precision upscale models.", modelId: "fal-ai/topaz/upscale/image" },
...
export function falImageInput(tool: StillTool, imageUrl: string, ratio: string, prompt = ""): { endpoint: string; input: Record<string, unknown> } {
  if (tool === "upscale") throw new Error("Upscaling requires reviewed source dimensions and upscale settings.");
