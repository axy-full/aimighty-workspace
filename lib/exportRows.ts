...
import { presignedReadUrl, usingBlob, videoPath, imagePath, audioPath } from "./storage";
...
export async function exportRows(): Promise<{ rows: ExportRow[]; unit: "cr" | "$" }> {
  ...
  for (const r of rs.rows as unknown as Record<string, unknown>[]) {
    const kind = r.kind === "image" ? "image" : r.kind === "audio" ? "audio" : "video";
    ...
    const hasMaster = r.status === "succeeded" && Boolean(r.stored_url);
    let url = "";
    if (hasMaster) {
      const pathname = kind === "image" ? imagePath(id) : kind === "audio" ? audioPath(id) : videoPath(id);
      url = blob ? await presignedReadUrl(pathname, 24).catch(() => `/api/media/${id}`) : `/api/media/${id}`;
    }
    rows.push({
