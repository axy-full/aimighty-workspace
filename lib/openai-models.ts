...
export async function openAIConnection(force = false, fetcher: typeof fetch = fetch): Promise<OpenAIConnection> {
  if (engineMock()) return { configured: false, verified: false, models: [], checkedAt: Date.now(), error: 'Live OpenAI verification is disabled in the mock environment.' };
  const key = vendorKey('openai');
  if (!key) return { configured: false, verified: false, models: [], checkedAt: Date.now(), error: 'No OpenAI API key is configured for this workspace.' };
  ...
  try {
    ...
    if (!response.ok) throw new Error(response.status === 401 || response.status === 403 ? 'OpenAI rejected this key or its model-list permission. Check the project key permissions.' : `OpenAI model verification is temporarily unavailable (${response.status}).`);
    const text = await response.text();
    if (text.length > 2_000_000) throw new Error('OpenAI returned an oversized model catalogue.');
    const body = JSON.parse(text);
    if (!Array.isArray(body.data) || body.data.length > 5000) throw new Error('OpenAI returned an invalid model catalogue.');
    ...
    result = { configured: true, verified: false, models: [], checkedAt: Date.now(), error: error instanceof Error && error.message.startsWith('OpenAI ') ? error.message : 'OpenAI could not be reached for model verification.' };
  }
