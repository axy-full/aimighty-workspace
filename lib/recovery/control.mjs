...
export class RecoveryFence {
  ...
  async finish(id, uncertain = false) {
    await this.ready();
    // These are outstanding-operation blockers, not the business audit ledger.
    // Completed HTTP/read/write activities must not grow a permanent table on
    // every request. Children and uncertain outcomes keep their own admissions.
    await this.client.execute(uncertain ? {
      sql: "UPDATE recovery_activities SET state='uncertain',finished_at=? WHERE id=? AND state='active'",
      args: [this.clock(), id],
    } : {
      sql: "DELETE FROM recovery_activities WHERE id=? AND state='active'",
      args: [id],
    });
  }
