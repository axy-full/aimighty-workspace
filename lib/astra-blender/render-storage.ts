    ...
    await workbenchTransaction(async (tx) => {
        ...
        for (const item of artifacts) {
            ...
            if (canAttach && project && !project.assets.some(asset => asset.id === item.assetId))
                project.assets.push({ id: item.assetId, name: `Astra ${item.filename}`, kind: item.kind === 'preview' ? 'image' : 'document', category: 'Astra Blender', url: item.url, mime: item.mime, uploadId: item.uploadId, description: 'Native Blender render output', prompt: '', status: 'Draft', locked: false, version: 1, refs: [] });
        }
