import { EMPTY_MOLECULR, marketingReferenceIds, moleculrNode, moleculrPrompt, moleculrReferences, type MoleculrGenerationOptions } from './moleculr';
...
export function buildMoleculrStoryboard(project: Project, createId: () => string, now = new Date().toISOString()): Project {
  ...
  const refs = moleculrReferences(project, brief, brief.castAssetIds[0]);
  ...
  const base = moleculrPrompt(project, brief, brief.hooks.find(hook => hook.trim()) ?? 'Introduce the product clearly.', brief.castAssetIds[0]);
  for (const [index, beat] of template.beats.entries()) {
    const id = uniqueId();
    const prompt = `${base}\n\nSHOT ${index + 1} OF ${template.beats.length} — ${beat.title}. Target duration ${beat.seconds} seconds. ${beat.prompt}\nKeep product, cast, light direction and screen direction consistent with the other campaign shots.`.slice(0, 12000);
    const node = moleculrNode(id, prompt, `${index + 1}. ${beat.title}`, next.nodes.length, 'video');
    const binding = bindMoleculrReferences(next, node, refs, uniqueId);
    next = { ...next, nodes: [...next.nodes, ...binding.sources, binding.node], moleculr: { ...next.moleculr!, variants: [...next.moleculr!.variants, { id, nodeId: id, hook: beat.title, kind: 'video', templateId: template.id, ...(brief.activeProductId && brief.productAssetIds.length ? { productId: brief.activeProductId } : {}), ...(brief.castAssetIds[0] ? { castAssetId: brief.castAssetIds[0] } : {}), createdAt: now, generation: { ratio: brief.creative?.aspect ?? template.aspect, duration: beat.seconds } }] } };
  }
  return next;
}
...
export function prepareMoleculrVariants(project: Project, kind: 'image' | 'video', createId: () => string, now = new Date().toISOString()): Project {
  ...
  for (const hook of hooks) for (const castId of cast) {
    const sourceIds = generation.marketing?.enhancePrompt ? marketingReferenceIds(project, brief, brief.productAssetIds[0], castId) : moleculrReferences(project, { ...brief, castAssetIds: castId ? [castId] : [] }, castId).map(asset => asset.id);
    const prompt = moleculrPrompt(project, brief, hook, castId).slice(0, kind === 'image' ? 5000 : 12000);
    if (brief.variants.some(variant => variant.kind === kind && variant.hook === hook && variant.castAssetId === castId && variant.productId === brief.activeProductId && variant.templateId === template?.id && JSON.stringify(variant.generation) === JSON.stringify(generation) && project.nodes.some(node => node.id === variant.nodeId && node.text === prompt))) continue;
    ...
    const id = uniqueId();
    const binding = bindMoleculrReferences(next, moleculrNode(id, prompt, `${brief.productName || project.name} · ${hook}`, next.nodes.length, kind), sourceIds.map(id => project.assets.find(asset => asset.id === id)!), uniqueId);
    next = { ...next, nodes: [...next.nodes, ...binding.sources, binding.node], moleculr: { ...next.moleculr!, variants: [...next.moleculr!.variants, { id, nodeId: id, hook, kind, ...(castId ? { castAssetId: castId } : {}), ...(brief.activeProductId ? { productId: brief.activeProductId } : {}), ...(template ? { templateId: template.id } : {}), generation, createdAt: now }] } };
    additions++;
  }
