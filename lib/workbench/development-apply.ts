...
export function applyDevelopment(project: Project, job: DevelopmentJob, choice: { idea: number } | { scenes: string[] }): Project {
  ...
  if (project.nodes.length + scenes.length > 250) throw new Error(`There is room for ${250 - project.nodes.length} more scene nodes. Add a smaller selection.`);
  ...
  const nodes: CanvasNode[] = scenes.map((scene, index) => {
    ...
    const node: CanvasNode = { id: uid('node'), title: scene.heading, type: 'scene', text, x: 50 + index % 4 * 350, y: Math.max(1030, ...project.nodes.map(value => value.y + 290)) + Math.floor(index / 4) * 290, width: 300, linked: [],
    ...
    if (node.y > 20000) throw new Error('Rearrange the canvas before adding more scenes.');
    return node;
  });
