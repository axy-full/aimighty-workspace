...
export type CanvasNode = {id:string; title:string; type:NodeType; assetId?:string; text?:string; x:number; y:number; width:number; linked:string[];operations?:NodeOperation[];bypassed?:boolean;locked?:boolean;collapsed?:boolean;role?:string;mode?:string;status?:'draft'|'review'|'approved';versions?:NodeVersion[];activeInput?:string};
...
export type Project = {productionProjectId?:string; shotMappings?:Record<string,string>; bibleVersion?:number; id:string; name:string; description:string; brief:string; audience:string; deliverables:string; direction:string; fps:number; aspect:string; assets:Asset[]; nodes:CanvasNode[]; shots:Shot[]; plans:Plan[]; briefPinned:boolean; lookPinned:boolean; createdAt:string; sharedAssetIds:string[]; sharedNodeIds:string[]; audioAssetId?:string; audioClips?:AudioClip[]; clipAudio?:boolean; script?:string; sharedNodes?:CanvasNode[]; sharedAssets?:Asset[]};
...
export function newProject(name:string):Project {const p=seedProject();return {...p,id:uid('project'),name,description:'New production',brief:'',script:'',audience:'',deliverables:'',direction:'',briefPinned:false,lookPinned:false,assets:[],nodes:[],shots:[],plans:[],sharedAssetIds:[],sharedNodeIds:[],sharedAssets:[],sharedNodes:[],createdAt:new Date().toISOString()};}
...
export function assetFilename(a:Asset){const ext:Record<string,string>={'image/png':'png','image/jpeg':'jpg','image/webp':'webp','image/gif':'gif','video/mp4':'mp4','video/webm':'webm','video/quicktime':'mov','audio/mpeg':'mp3','audio/wav':'wav','audio/x-wav':'wav','audio/mp4':'m4a','audio/ogg':'ogg','application/pdf':'pdf','text/plain':'txt'};const suffix=ext[a.mime||'']||a.url.split('?')[0].match(/\.([a-zA-Z0-9]{2,5})$/)?.[1]||(a.kind==='video'?'mp4':a.kind==='audio'?'mp3':'png');return safeName(a.id)+'_'+safeName(a.name.replace(/\.[a-zA-Z0-9]{2,5}$/,''))+'.'+suffix;}
...
export function demoPlan(request:string,p:Project,model:string,depth:string,refs:string[]):Plan {
 ...
 return {id:uid('plan'),request,model,depth,intent,summary:intent==='continuity'?'A continuity review to protect the visual decisions already made.':intent==='revision'?'A separate revision keeps the selected take intact.':intent==='shots'?'A three-beat structure: establish the world, introduce the character, leave an impression.':`A production plan for ${p.name}, built around a repeatable visual world.`,steps,applied:false,refs};
}
