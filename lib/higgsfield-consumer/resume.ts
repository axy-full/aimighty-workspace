 ...
 * jobs are still in flight, what each state is called, and when to ask again.
 *
...
/** Submitted and not yet settled: the account may be rendering it. */
export const RESUMABLE_STATUSES: readonly ResumeStatus[] = Object.freeze(["dispatching", "accepted", "uncertain"]);
export const isResumable = (status: string) => (RESUMABLE_STATUSES as readonly string[]).includes(status);
export const isSettled = (status: string) => status === "completed" || status === "failed";

/** Jobs still in flight, newest first; `accept` narrows them to one composer's own. */
export function resumableJobs<J extends { status: string; createdAt: number }>(jobs: readonly J[], accept?: (job: J) => boolean): J[] {
  return jobs.filter((job) => isResumable(job.status) && (!accept || accept(job))).sort((a, b) => b.createdAt - a.createdAt);
}

export type ResumePhase = { label: string; tone: "blue" | "green" | "red" | "amber" | "idle"; pct: number };
/** One word per state, shared by the Gen cards, Business and Viral rows. */
export function resumePhase(status: string): ResumePhase {
  switch (status) {
    case "dispatching": return { label: "Submitting", tone: "blue", pct: 10 };
    case "accepted": return { label: "Rendering", tone: "blue", pct: 50 };
    /* The account has not confirmed it yet. It is never sent twice. */
    case "uncertain": return { label: "Confirming", tone: "amber", pct: 30 };
    case "completed": return { label: "Complete", tone: "green", pct: 100 };
    case "failed": return { label: "Failed · not billed", tone: "red", pct: 100 };
    /* Priced, never sent: nothing was spent. */
    default: return { label: "Quoted · not sent", tone: "idle", pct: 0 };
  }
}
...
export function resumeProblem(code: string | null | undefined, message: string | null | undefined): string {
  if (code === "reconnect_required" || code === "connection_changed")
    return "Reconnect the account in Workspace › Engines to finish this take.";
...
export type ResumeTracker<J> = {
  /** Start following each in-flight job not already followed. */
  track: (jobs: readonly J[]) => void;
  /** Stop following one job (it is shown elsewhere, or was dismissed). */
...
/**
 * Follow several in-flight jobs at once, each on its own clock: ask, report,
 * and ask again until it settles. One job's error never stops another's.
 */
export function createResumeTracker<J extends { id: string; status: string }>(deps: {
  ...
  return {
    track(jobs) {
      if (stopped) return;
      for (const job of jobs) if (isResumable(job.status) && !timers.has(job.id)) follow(job.id, job.status, deps.firstDelayMs ?? 1200, 0);
    },
    forget(id) {
