 ...
 * Where mail works, a new account is made only from the emailed link (its
 * `m` proof), since whoever sent the invitation can also read its code.
 */
...
type Info = { workspace: string; email: string; name: string; role: string; hasAccount: boolean; signedInAsInvitee: boolean; mailboxNeeded?: boolean };

export default function InvitePage({ params, searchParams }: { params: Promise<{ code: string }>; searchParams: Promise<{ m?: string | string[] }> }) {
  const { code } = use(params);
  const raw = use(searchParams).m;
  const proof = typeof raw === "string" ? raw : "";
  ...
  useEffect(() => {
    let live = true;
    fetch(`/api/auth/accept?code=${encodeURIComponent(code)}${proof ? `&m=${encodeURIComponent(proof)}` : ""}`)
  ...
  async function accept(withPassword: boolean) {
    ...
    try {
      const res = await fetch("/api/auth/accept", {
        ...
        body: JSON.stringify(withPassword ? { code, m: proof, password, accept: agreed } : { code }),
      });
      const json = await res.json().catch(() => ({}));
      if (json.needsMailbox && info && !("dead" in info)) { setInfo({ ...info, mailboxNeeded: true }); setBusy(false); return; }
  ...
  async function emailLink() {
    ...
    try {
      const res = await fetch("/api/auth/accept", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code, emailLink: true }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json.error ?? `The server answered ${res.status}. Try again in a moment.`);
      setMailed(true);
    } catch (e) {
      setErr((e as Error).message);
    } finally {
  ...
  if (!info.hasAccount && info.mailboxNeeded) {
    return (
      <AuthCard title={`Join ${info.workspace}`} sub={`Choose your password from the invitation email sent to ${info.email}.`}>
        {mailed
          ? <p role="status" className="page-sub">Sent. Open the link in that email.</p>
          : <button type="button" className="btn-primary !h-[46px] w-full justify-center !text-[14px]" onClick={emailLink} disabled={busy}>{busy ? "…" : "Email me the link"}</button>}
