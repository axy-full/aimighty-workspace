...
import { platformDb, platformReady, findAccountByEmail, getWorkspace, now } from "@/lib/platform";
import {acceptWorkspaceInvitation} from "@/lib/teamInvitations";
import {accountFailure,accountJson,sameOriginProblem,AccountError} from "@/lib/accountDb";
...
async function lookup(code: string) {
  ...
  if (!ws || ws.deletedAt) return { problem: "That workspace no longer exists.", status: 410 } as const;
...
export const POST = recoveryRoute(async function POST(req: Request) {
  ...
  try{
    ...
    const joined=await acceptWorkspaceInvitation({code:String(body.code??'').trim(),password:String(body.password??''),name:String(body.name??''),signedInAccountId:ctx?.user.id});
    const token=await createSession(joined.accountId,joined.workspace.id);
    (await cookies()).set(SESSION_COOKIE,token,{httpOnly:true,sameSite:'lax',secure:process.env.NODE_ENV==='production',path:'/',maxAge:30*86400});
    return Response.json({ok:true,workspace:{id:joined.workspace.id,name:joined.workspace.name}});
  }catch(error){
