...
import { SESSION_COOKIE, currentContext, sourceKey } from "@/lib/auth";
...
export const POST = recoveryRoute(async function POST(req: Request) {
  ...
  try {
    ...
    if (code) {
      ...
      const accepted = await acceptSignupInvitation(
        { ...input, code },
        ctx?.user.id,
        signedInSession,
      );
      // The account and resumable request survive every subsequent resource error.
      const { session, created } = await accountInvitationSession(
        accepted.owner.id,
        ctx?.user.id,
        signedInSession,
      );
      ...
      if (result.workspace) {
        await switchSessionWorkspace(session, result.workspace.id);
    ...
    return Response.json(
      {
        ...
        message:
          "Check your email to verify your address. No credits or subscription are charged until you choose and pay for a plan.",
      },
      { status: 202 },
    );
  } catch (error) {
...
export const GET = recoveryRoute(async function GET(req: Request) {
  ...
  const inv = (
    await platformDb().execute({
      sql: "SELECT email,name,used_at,expires_at FROM signup_invites WHERE code=?",
      args: [code],
    })
  ).rows[0];
  if (!inv)
  ...
  if (inv.used_at)
    return Response.json(
      {
        error:
          "This invitation has already been used. Sign in to continue your workspace setup.",
      },
      { status: 409 },
    );
  ...
  return Response.json(
    {
      ...
      email: inv.email,
      name: inv.name,
      open: workspaceCreationReadiness().canCreate,
    },
    { headers: { "Cache-Control": "no-store" } },
  );
});
