...
export async function GET() {
  ...
  const [totals, topups, byModel, byProject, byPerson, byMonth, recent] = await Promise.all([
    db().execute(`
      SELECT COUNT(*) AS n,
    ...
    db().execute(`
      SELECT strftime('%Y-%m', datetime(created_at/1000,'unixepoch')) AS month,
             COUNT(*) AS n, COALESCE(SUM(cost_usd),0) AS spend
      FROM generations WHERE status='succeeded'
      GROUP BY month ORDER BY month DESC LIMIT 12`),
    db().execute(`
      SELECT id, model, prompt, cost_usd, total_tokens, params, created_at
  ...
  return NextResponse.json({
    ...
    recent: recent.rows.map((r: any) => ({
      ...
      prompt: r.prompt, costUsd: Number(r.cost_usd),
