 ...
 * server (§7A); `legacyCeiling` only says an older dollar ceiling, set
 * before credits, still applies. A workspace on its own keys keeps dollars.
 */
export const GET = withTenant(async function GET() {
  const user = await currentUser();
  if (!user) return NextResponse.json({ error: "Sign in to manage tokens" }, { status: 401 });
  await ready();
  const credits = creditsApply(requireTenant().workspace);
  ...
  const rs = await db().execute({
    sql: `SELECT t.id, t.name, t.scope, t.cap_usd, t.cap_credits, t.last_used, t.created_at,
                 ...
                 (SELECT ${billedCreditsSum("g")} FROM generations g
                   WHERE g.token_id = t.id AND g.created_at >= ?) AS spend_credits
          FROM api_tokens t
          WHERE t.user_id = ? AND t.revoked_at IS NULL
          ORDER BY t.created_at DESC`,
    args: [start.getTime(), start.getTime(), user.id],
  });
    ...
    tokens: rs.rows.map((r: any) => ({
      ...
      ...(credits
        ? {
            ...
            spendCredits: Number(r.spend_credits ?? 0),
...
export const POST = withTenant(async function POST(req: Request) {
  ...
  /* A ceiling in credits is a whole number, and only means something in a
     workspace that pays in credits and on a token that can spend. */
  const creditCeiling = scope === "render" && creditsApply(requireTenant().workspace);
  const creditsRaw = Number(body.capCredits);
  const capCredits = creditCeiling && Number.isInteger(creditsRaw) && creditsRaw > 0 && creditsRaw <= 1_000_000 ? creditsRaw : null;
  if (creditCeiling && body.capCredits != null && body.capCredits !== "" && capCredits == null)
    return NextResponse.json({ error: "A ceiling is a whole number of credits, 1 to 1,000,000." }, { status: 400 });
