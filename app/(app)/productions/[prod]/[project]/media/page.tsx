...
import { Segmented, Chip, Button, Mono } from "@/components/ui";
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-ground text-ink">
      ...
      <div className="flex h-[52px] flex-none items-center gap-[14px] px-[24px] max-md:h-auto max-md:flex-wrap max-md:gap-[10px] max-md:px-[16px] max-md:py-[10px]">
        ...
        <span className="ml-auto flex items-center gap-[10px]">
          <label className="flex h-[36px] w-[220px] flex-none items-center gap-[8px] rounded-pill border border-border bg-card px-[12px] text-[13px] text-ink-muted max-md:w-full">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-[20px] overflow-auto px-[24px] pb-[24px] max-md:px-[16px]">
        {groups.map((g) => (
          <section key={g.id} className="flex flex-col gap-[10px]">
            <div className="flex items-baseline gap-[10px]">
              <Mono tone="ink">{g.code}</Mono>
              {g.title && <span className="text-[14px] font-semibold leading-[1.2] text-ink">{g.title}</span>}
              <span className="text-[12.5px] leading-none text-ink-body">{g.items.length} {g.items.length === 1 ? "item" : "items"} · {fmt(g.items.reduce((a, m) => a + (money.inCredits ? m.credits : m.usd), 0))}</span>
              <span className="h-px flex-1 self-center bg-[rgba(245,246,248,.07)]" />
              {g.id !== "unfiled" && <Link href={`/shots/${g.id}`} className="ui-mono text-ink-muted">Open shot →</Link>}
            </div>
            <div className="grid grid-cols-6 gap-[10px] max-md:grid-cols-2">
              {g.items.map((m) => <MediaTile key={m.id} m={m} onOpen={() => { if (m.url) window.open(m.url, "_blank", "noopener"); }} />)}
            </div>
          </section>
        ))}
