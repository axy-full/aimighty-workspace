...
## Consumer video originals and workspace deletion

Consumer Marketing Video collection commits private original bytes and a digest receipt before the separate consumer job completion. Until that completion, matching originals cannot be deleted and are excluded from cleanup batches. A retry reuses the recorded original without another generation request.

After completion, ordinary guarded Library deletion may remove an unattached original. Job views check its current tenant generation and storage receipt separately from the immutable completion record. Deleted, missing or mismatched originals have no download/attachment descriptor; their historical receipt and quoted credits remain intact. This metadata availability check does not replace the authenticated media read or the transactional attachment/deletion guards.

Explicit workspace deletion provides a separate disposal path through ordinary `purgeWorkspace`. After its ten-minute grace period and any live consumer poll lease, one tenant write transaction verifies each stored original's job identity and immutable receipt, then records a completed, non-attachable `workspace_deleted` disposal with state `purge_pending`. This records the requested disposition, not provider failure, a refund, or confirmed physical deletion. The original receipt, digest and quoted Higgsfield credits remain unchanged until the independently retryable file and database purge succeeds. A failed cleanup resumes its unfinished stages without another provider call or disposal rewrite.

Preparing receipts with reserved bytes or live leases remain blocked, including after workspace deletion. There is currently no automatic recovery endpoint for an ambiguous original write in a deleted tenant; an operator must reconcile that retained storage receipt before purge can proceed. Lease expiry alone does not establish that a write failed or release its quota.
