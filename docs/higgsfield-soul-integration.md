...
**Cast & Elements is identity-first (four-suites PR F, 19 September 2026).** The product never names the provider: the feature is **Identity** everywhere in the stage, the identity panel and the generation dialog (the settings connection card under Workspace → Engines still names Higgsfield, because an owner connects their own account there). Each cast or element card reads its state from the workspace identity list — *No identity · Identity training · Identity ready · Identity failed* (an `uncertain` training shows as failed until reconciled; a binding the list does not return shows *Identity attached*) — and carries an **Identity** action that opens the panel for that asset. The stage toolbar keeps **Identity** (cast) and **Element identity**. The older LoRA identity trainer (`lib/identities.ts`, `/api/identities`) no longer has an entry point in the Particl suite; its routes, rows and tests stay for accepted trainings, and it remains reachable only from the Library, Rig canvas, Shots and Make asset sheet, marked legacy in code.

A ready identity is the default reference for every take of its character or element: the attached asset carries the identity's original portrait, so any generate node linked to that cast/element node lists it under **Bound references** as `<name> · Identity`. While Soul Character generation stays gated (`HF_SOUL_CHARACTER_ENABLED` unset or unpriced), `/api/workbench/engines` lists no identity engine and the generation dialog says, in one line, that identity rendering is awaiting verification and the take uses the identity's portrait as its reference through the current image engines. Training is unchanged: $2.50 per request through `billCredits`, quoted and admitted exactly as before (`lib/soulIdentities.ts` economics untouched).

Open Cast & Elements, choose **Identity**, and select uploaded or completed generated portraits of one character. Particl accepts 1–40 JPEG, PNG or WebP originals; Higgsfield's API permits up to 100. Several clear views provide better identity coverage than a single image. Soul ID is not a prop, product or environment trainer.

Review the server quote and rights consent, then create the identity. Training remains visible after leaving or reloading. When ready, attach it as a normal character/element asset and add that asset to the canvas. With the verified generation gate enabled, the node's generation dialog offers the identity engine (model `hf-soul-character`, labelled **Identity render**) with an **Identity** selector and likeness strength. Other engines continue to use its original portrait reference. Generation remains gated until the paid verification (`docs/four-suites-v2-plan.md`, qualifications) confirms access and prices. Completed images enter the existing takes/library and full-resolution download flow.
