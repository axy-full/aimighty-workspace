...
- Recovery: enforced writer/worker/provisioning/purge quiescence, complete automatic source inventory, separate archive key escrow, scheduled encrypted capture, freshness failure detection and successful independent restore. The isolated recovery-fence increment now implements durable admissions, exact accepted-job draining, complete source discovery and live receipt checks (`docs/enforced-recovery-fence.md`). Production activation still requires the full supported deployment inventory and verified exclusion of old/manual writers; the existing `quiesced` JSON alone remains insufficient.
