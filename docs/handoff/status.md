...
| History = this project's Genjutsu results (from the route's GET): Recreate (the job's own inputs resolved against the Library and loaded into the composer, priced again), Compare (original and result on one clock, in a sheet), Send to Edit (finds the collected original in the project's Library — paging back past the loaded pages, since an original is filed at its run's own time — then selects it and opens Studio › Takes, where a take is re-edited; not found, it only says so). Takes opens at the take handed over, in view; a handed-over take not yet loaded shows "Finding the take…", never another take | `ViralView.tsx › HistoryView`, `CompareSheet`, `lib/workspace/library.ts › findProjectTake`, `EditStage.tsx` |
| **Real runs only** (26 September): Recent and History read the route's `view=runs` — every job past its quote, newest first, 24 a page by cursor (`Load older runs`), with every job still awaiting reconciliation on the first page; Recent beside a composer asks for its own variant (`&variant=`). Estimates stay in the ledger and in the default GET (the quote-driven surfaces still price from it). Each state reads as words (Queued · Rendering · Checking · Failed · not billed · Done), with its meaning on the card, not in a tooltip; a finished card whose original is gone says Archived or Original unavailable and disables Compare and Send to Edit. Every run in flight — including ones sent before the page opened — is read until it lands, at most one read per 4 s tick, a rendering run at the account's own `pollAfterSeconds`, others backing off to 60 s; a run nothing can move (Queued, or Checking with no receipt) is read three times, a run the route cannot find once, then each offers Check again. Copies merge by how far along they are, then `updatedAt`, so a stale read never sets a run back. A landed run re-reads the project's Library once; a finished Recent row opens in Takes. Loading, empty, error (Try again — in the composer too, whose reason reads "Reading the connected account…" or the read error, never a false connect hint) and owner-only states | `lib/higgsfield-consumer/jobs.ts › listConsumerRuns`, `genjutsu-service.ts › consumerGenjutsuRuns`, `lib/shell/use-viral.ts`, `lib/shell/viral.ts › RUN_STATUS, mergeRuns` |
| Viral pages are the shell's own views (`own: true`) | `lib/shell/ia.ts`, `SuitesShell.tsx` |

Tests: `tests/unit/suitesViral.spec.ts`, `tests/unit/viralRealRuns.spec.ts`, `tests/suites-viral-workbench.spec.ts`, `tests/hf-viral-real-runs-workbench.spec.ts` (five viewports).
