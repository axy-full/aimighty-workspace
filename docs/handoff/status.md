...
| History = this project's Genjutsu results (from the route's GET): Recreate (the job's own inputs resolved against the Library and loaded into the composer, priced again), Compare (original and result on one clock, in a sheet), Send to Edit (selects the collected original and opens Studio › Takes, where a take is re-edited) | `ViralView.tsx › HistoryView`, `CompareSheet` |
| **Real runs only** (26 September): Recent and History read the route's `view=runs` — every job past its quote, newest first, 24 a page by cursor (`Load older runs`), with every job still awaiting reconciliation on the first page. Estimates stay in the ledger and in the default GET (the quote-driven surfaces still price from it). Each state reads as words (Queued · Rendering · Checking · Failed · not billed · Done); every run in flight — including ones sent before the page opened — is read until it lands, one per tick; a landed run re-reads the project's Library once. Loading, empty, error (Try again) and owner-only states | `lib/higgsfield-consumer/jobs.ts › listConsumerRuns`, `genjutsu-service.ts › consumerGenjutsuRuns`, `lib/shell/use-viral.ts`, `lib/shell/viral.ts › RUN_STATUS, mergeRuns` |
