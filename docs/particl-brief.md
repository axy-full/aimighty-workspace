...
7. **Desktop first; the phone is the floor.** Revised 8 September 2026. The product is used at a desk, on wide screens, and every surface is designed for that first — density, rails and reading measure are judged at 1440 and above, not scaled up from a phone. The phone does not stop mattering: producers still approve takes from one, so every change ships with Playwright checks at 360×640, 390×844 and 844×390, and no route may have horizontal overflow, an unreachable primary action, or a dock under the home indicator. That suite is a floor to clear, not the shape to design to.
