...
| 12e Deliver + take rail | `…/media` | the Media tab, `/api/export` (zip, CSV), review links (`/api/shares`), provenance (`take_ports`) | the Deliver band (masters named `{production}_{shot}_{version}`, CSV with Made-from, EDL/FCPXML rebuilt, review link minted from the tab); zip via signed Blob URLs; the take rail with Made from as links and the priced post tools that do not ask for a reason on an approved take — reframe, upscale, extend, and **lip-sync** (sync-3 is registered and wired through `/api/generate` since CR1 item 4; the take rail is its first surface) | ~9 d |
