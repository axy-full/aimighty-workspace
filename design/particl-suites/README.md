...
### Shell (every screen)
- **Header**: particl trail mark (7 growing dots, `viewBox 30 68 140 64`, fill `#F5F5F7`, 30×14) + wordmark 15/600 → Studio. Suite tabs as a segmented control. Search field (flex 0 1 460px, inset look) opens the `⌘K` palette. Right: running-jobs pill (blue pulsing dot; counts every job in flight or held from any page and either engine, "3 rendering · 1 held", or what finished unseen; opens the **Jobs tray** — a popover under the pill, a bottom sheet on a phone — one row per job: real stage, age, approved price, one action: Open in Takes · Release · Recreate), credits pill (mono 600, → Workspace › Plans & credits), avatar/workspace menu.
