...
async function fixture(page:Page) {
  ...
  const saved=await save(project);await save(second);
  ...
  const generation=`render_${randomUUID().replaceAll('-','')}`;
  try {
    await mkdir('.data/generations',{recursive:true});await mkdir('.data/uploads',{recursive:true});
    await writeFile(`.data/generations/${generation}.png`,bytes);await writeFile(`.data/uploads/${privateId}.png`,bytes);
  ...
  return{project,second,original,other,generation,privateId,scope,headers,bytes};
}
