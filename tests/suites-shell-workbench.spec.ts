...
test("the shell lands on Studio with the README's header, strip and columns", async ({ page }, info) => {
  ...
  await expect(strip.getByRole("button")).toHaveText([/^01\s*Brief$/, /^02\s*Beats$/, /^03\s*Storyboards$/, /^04\s*Cast$/, /^05\s*Astra$/, /^06\s*Rig$/, /^07\s*Takes$/, /^08\s*Edit & Sound$/, /^09\s*Deliver$/]);
