...
test("the shell lands on Studio with the README's header, strip and columns", async ({ page }, info) => {
  ...
  await expect(strip.getByRole("button")).toHaveText([/^01\s*Brief$/, /^02\s*Beats$/, /^03\s*Storyboards$/, /^04\s*Cast$/, /^05\s*Astra$/, /^06\s*Rig$/, /^07\s*Edit$/, /^08\s*Timeline$/, /^09\s*Delivery$/]);
...
test("⌘K finds a page, runs the top hit on Enter and closes on Escape", async ({ page }, info) => {
  ...
  await expect(dialog.getByRole("option").first()).toContainText("09 Delivery");
