 ...
 * server: the changed project pages at every size: nothing scrolls sideways.
 * Nothing here submits paid work. The audit is spread over files whose names
 * sort apart, because CI shards take contiguous runs of files and each legacy
 * page costs its dev server gigabytes to compile.
 */

test("changed project pages do not scroll sideways", async ({ page }) => {
