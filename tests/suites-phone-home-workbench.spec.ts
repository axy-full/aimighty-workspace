...
test("phone: the Studio tab opens the home; a card opens its page; ‹ Studio comes back; the floors hold", async ({ page }, info) => {
  ...
  await expect(page.getByRole("navigation", { name: "Pages" }).getByRole("button", { name: /Studio/ })).toHaveCount(0);
...
test("desktop: the strip has no Studio home tab and the header no back button", async ({ page }, info) => {
