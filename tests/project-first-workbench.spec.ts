...
test('Library and Production navigation retain the selected project',async({page},info)=>{
 ...
 await expect(page.locator('#project-name')).toHaveValue('Project first fixture');
 await expect(page.getByRole('navigation',{name:'Particl Production Studio pages',exact:true}).getByRole('link',{name:'Brief & Script',exact:true})).toHaveAttribute('aria-current','page');expect(f.errors).toEqual([]);
});
