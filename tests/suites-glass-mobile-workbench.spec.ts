...
test("phone: Home is the suite picker; the tab bar and top bar float as glass; Studio opens the grid with a Home back; sheets are glass; the floors hold", async ({ page }, info) => {
  ...
  await expect(tiles).toContainText(["Brief to delivery, eight stages.", "Video, images, audio, 3D — one composer.", "Marketing Studio: product, presenter, ad.", "Genjutsu: motion transfer, object swap.", "Plans, prices, waits for your word.", "One Grok agent per department."]);
  /* The brief has words and one take exists: Brief and Takes are done. */
  await expect(page.getByTestId("home-fact-studio")).toHaveText("2 of 8 done");
