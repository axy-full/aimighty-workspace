 ...
 * Every reply is route-mocked; nothing paid is ever sent.
 */
...
async function open(page: Page, url: string, library: { pageSize?: number; failFirst?: () => boolean } = {}) {
  await signInLocally(page.request);
  await forbidPaidWork(page);
  await mockMedia(page);
  await mockProjects(page, { current: fixture() });
  await mockLibrary(page, { uploads: [upload({ id: "up_plate", filename: "harbour-plate.webp" })], generations: takes, pageSize: library.pageSize });
...
test("Ads: a failed quote re-arms Generate on its own, and a finished ad can be made again", async ({ page }, info) => {
  ...
  await expect(page.getByTestId("ads-error")).toHaveText("The account is busy. Try again shortly.");
