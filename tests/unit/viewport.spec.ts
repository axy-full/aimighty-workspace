...
/**
 * The Rig canvas viewport (docs/change-request-1.md §6): zoom about the
...
test("zoom is clamped to 25%–200% and a no-op returns the same view", () => {
  ...
  expect(clampZoom(0.1)).toBe(0.25); expect(clampZoom(3)).toBe(2); expect(clampZoom(1.5)).toBe(1.5);
});
