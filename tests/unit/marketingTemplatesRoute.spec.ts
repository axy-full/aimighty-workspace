...
const input = { presetId: "tpl_product_shot_studio", prompt: "A plain bottle.", brandName: "Our bottle", productImage: { uploadId: "original-still" } };
...
async function fixture() {
  ...
  const job = { id: key, draftId: "draft-1", workflow: "marketing-template", status: "quoted", quoteCredits: 40, creditUnit: "higgsfield_credits", template: { id: "tpl_product_shot_studio", name: "Studio product shot", category: "product-shot", previewUrl: null }, outputKind: "image", priceSource: "cost_table" };
  const catalogue = { templates: [], total: 6, complete: true, fetchedAt: 1 };
  const table = { version: "2026-09-18", entries: [{ key: "default", credits: 30 }], fetchedAt: 1 };
  ...
  const deps: Record<string, unknown> = {
    ...
    "@/lib/higgsfield-consumer/marketing-template-service": {
      consumerMarketingTemplateJobs: service("list", [job]),
      connectedMarketingTemplateCatalogue: service("catalogue", catalogue),
      connectedMarketingTemplateCosts: service("costs", table),
      presentMarketingTemplates: (value: unknown, prices: unknown, options: unknown) => ({ value, prices, options }),
      quoteConsumerMarketingTemplate: service("quote", job),
      submitConsumerMarketingTemplateJob: service("submit", { ...job, status: "accepted" }),
      pollConsumerMarketingTemplate: service("status", { job: { ...job, status: "accepted" } }),
    },
  };
  ...
  return {
    calls, limits, connections, connection, job, catalogue, table, store: () => store, setStore: (value: tenant.TenantStore) => { store = value; },
    ...
    request: async (method: "GET" | "POST", body: unknown = quote, options: { scope?: string | null; origin?: string; query?: string; raw?: string | Uint8Array; contentLength?: string; empty?: boolean } = {}) => {
      ...
      const request = new Request(`https://particl.example/api/higgsfield/consumer/marketing-templates${options.query ?? ""}`, {
...
test("template routes reject signed-out, nonowner and API-token callers before service access", async () => {
...
test("template requests cannot adopt another workspace, user, origin or incomplete MFA session", async () => {
  const f = await fixture(), original = f.store();
...
test("template body validation bounds actual UTF8 bytes and reports malformed JSON without leaking content", async () => {
  const f = await fixture();
...
test("template errors preserve bounded categories and never expose provider or storage details", async () => {
  ...
  for (const [error, expected] of [[new ConsumerOAuthError("reconnect_required"), 401], [new ConsumerDiscoveryError("timeout"), 504],
    ...
    [new ConsumerVideoServiceError("not_found", "This template job is not available.", 404), 404],
    ...
    [new templates.MarketingTemplateError("template_unknown", "Choose a template from the connected catalogue."), 400],
    [new templates.MarketingTemplateError("price_unknown", "The catalogue's cost table has no price for this template. Nothing was submitted."), 409],
    [new templates.MarketingTemplateError("contract_unverified", "The connected account's create tool does not advertise the arguments this workflow sends. Nothing was submitted."), 502],
    [new templates.MarketingTemplateError("invalid_catalogue", "The connected account returned an unusable template catalogue."), 502],
