...
/** Particl Mobile.dc.html › STUDIO HOME: one card per stage (nine with Beats, owner's brief 23 September), every line from the project. */
...
test("every card's line and dot come from the project and the library", () => {
  ...
  expect(cards.map((c) => c.status)).toEqual(["ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready"]);
  expect(cards.find((c) => c.id === "beats")?.meta).toBe("no beats yet");
  ...
  expect(live.find((c) => c.id === "edit")).toMatchObject({ meta: "2 shots · 0 clips", status: "done" });
...
test("the Home tiles carry the prototype's lines and live facts; the Assets row counts the project", () => {
  ...
  expect(tiles.map((t) => t.line)).toEqual([
    "Brief to delivery, nine stages.", "Video, images, audio, 3D — one composer.", "Marketing Studio: product, presenter, ad.",
    "Genjutsu: motion transfer, object swap.", "Plans, prices, waits for your word.", "One Grok agent per department.",
  ]);
  expect(tiles.map((t) => t.fact)).toEqual(["1 of 9 done", "Seedance 2.5 ready", "UGC · 15 s · quoted in Ads", "720p · quoted on the source", "2 awaiting approval", "7 seats"]);
