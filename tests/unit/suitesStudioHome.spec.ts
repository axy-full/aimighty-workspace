...
test("every card's line and dot come from the project and the library", () => {
  ...
  expect(cards.map((c) => c.status)).toEqual(["ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready", "ready"]);
  expect(cards.find((c) => c.id === "beats")?.meta).toBe("no beats yet");
  expect(cards.find((c) => c.id === "environment")?.meta).toBe("no world yet");
...
test("the Home tiles carry the prototype's lines and live facts; the Assets row counts the project", () => {
  ...
  expect(tiles.map((t) => t.line)).toEqual([
    "Brief to delivery, ten stages.", "Video, images, audio, 3D — one composer.", "Marketing Studio: product, presenter, ad.",
    "Genjutsu: motion transfer, object swap.", "Plans, prices, waits for your word.", "One Grok agent per department.",
  ]);
  expect(tiles.map((t) => t.fact)).toEqual(["1 of 10 done", "Seedance 2.5 ready", "UGC · 15 s · quoted in Ads", "720p · quoted on the source", "2 awaiting approval", "7 seats"]);
