...
export function localPlatformDbUrl() {
  ...
  if (!url.startsWith("file:")) throw new Error("Browser fixtures require an explicitly local platform database.");
  return url;
}
...
export async function signInLocally(api: APIRequestContext) {
  ...
  test.skip(!/^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?\/?$/.test(base), "requires an explicitly local server");
  const health = await api.get("/api/health", { timeout: 15_000 }).then(response => response.json());
