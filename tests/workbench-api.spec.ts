...
test("real local routes persist a single generated take and isolate another workspace", async ({ request, playwright }) => {
  ...
  for (const scope of [`particl-active-${me.workspace.id}-previous-account`, `particl-active-previous-workspace-${me.id}`]) {
    const stale = { "X-Workbench-Scope": scope };
    expect((await request.put("/api/workbench/projects", { headers: stale, data: { project: draft, revision: 0 } })).status()).toBe(409);
    for (const action of ["publish", "map-shot", "open"]) {
      expect((await request.post("/api/workbench/projects", { headers: stale, data: { action, projectId: draft.id, nodeId: "api-shot" } })).status()).toBe(409);
    }
    expect((await request.get(`/api/workbench/projects?id=${draft.id}`).then(response => response.json())).project).toBeNull();
    expect((await request.get("/api/projects").then(response => response.json())).projects).toHaveLength(0);
  }
  const saved = await request.put("/api/workbench/projects", { headers: scopeHeaders, data: { project: draft, revision: 0 } });
  ...
  const mapped = await request.post("/api/workbench/projects", { headers: scopeHeaders, data: { action: "map-shot", projectId: draft.id, nodeId: "api-shot" } });
  ...
  const headers = { "Idempotency-Key": randomUUID(), ...scopeHeaders };
  ...
  const revised = await request.put("/api/workbench/projects", { headers: scopeHeaders, data: { project: loaded.project, revision: loaded.revision } });
  ...
  expect((await request.put("/api/workbench/projects", { headers: scopeHeaders, data: { project: loaded.project, revision: loaded.revision } })).status()).toBe(409);
  ...
  try {
    ...
    expect((await outsider.post("/api/workbench/projects", { headers: otherHeaders, data: { action: "open", projectId: identity.productionProjectId } })).status()).toBe(404);
    expect((await outsider.put("/api/workbench/projects", { headers: otherHeaders, data: { project: loaded.project, revision: 0 } })).status()).toBe(409);
